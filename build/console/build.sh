...
component=software-distribution-platform-console
version=${1:-"v0.0.1"}
...
function prepare_build_file(){
    mkdir -p "${OUTPUTDIR}"
    cp -rf "${PROJECT_ROOT}/build/console/charts" "${OUTPUTDIR}/"
    cp -rf "${PROJECT_ROOT}/build/console/images" "${OUTPUTDIR}/"
}

# ----- 版本渲染（2026-09-21 新增）-----
# 背景：此前 build.sh 只把 version 用在镜像 tag / 文件名两处，
...
function build_console_docker_image(){
